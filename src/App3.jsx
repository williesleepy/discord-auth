...
export default function App() {
  ...
  async function loadPcloudFolder(folderPath) {
    ...
    try {
      ...
      if (!res.ok) throw new Error("pCloud folder fetch failed");
  ...
  return (
    <div style={{ padding: 20 }}>
      ...
      {tab === "files" && (
        <div>
          <h3>pCloud Thumbnail Explorer</h3>
            ...
            <>
              ...
              <div style={{ marginBottom: 24 }}>
                {pcloudFolders.map((folder) => {
                  ...
                  return (
                    <button
                      onClick={() => loadPcloudFolder(folderPath)}
...
function ThumbnailCard({ item, path }) {
  ...
  useEffect(() => {
    ...
    async function loadThumb() {
      ...
      try {
        const res = await fetch(
          `${API}/pcloud/thumb?path=${encodeURIComponent(path)}&size=256x256`,
          {
            headers: {
  ...
  return (
    ...
    >
      ...
      >
        ...
        {!thumbUrl && !failed && <span>Loading...</span>}
